...
int printing(FILE *ptr, struct items *product, struct items *cart, int productQuantity)
{
    ...
    while (fscanf(ptr, "%d %s %d %d", &product[i].code, product[i].name, &product[i].quantity, &product[i].price) != EOF)
    {
        ...
        if (m == 1)
        {
            printf("\n%s is running out of stock", product[i].name);
        }
    ...
    for (int i = 0; i < productQuantity; i++)
    {
        ...
        for( int j = 0; j < productQuantity - 1; j++)
        {
            if(cart[j].price < cart[j+1].price)
            {
                int temp1;
                temp1 = cart[j].code;
                cart[j].code = cart[j+1].code;
                cart[j+1].code = temp1;
                int temp2;
                temp2 = cart[j].price;
                cart[j].price = cart[j+1].price;
                cart[j+1].price = temp2;
                int temp3;
                temp3 = cart[j].quantity;
                cart[j].quantity = cart[j+1].quantity;
                cart[j+1].quantity = temp3;
                char temp4[20];
                strcpy(temp4,cart[j].name);
                strcpy(cart[j].name,cart[j+1].name);
                strcpy(cart[j+1].name,temp4);
            }
        }
            if (cart[i].quantity != NULL)
            {
                printf("   %i\t%s\t\t%i\t\t%i", cart[i].code, cart[i].name, cart[i].quantity, cart[i].price,cart[i].quantity);
            }
        // if(cart[i].quantity != NULL
            printf("\n");
        // printf("\n");
    ...
    if (userCode == 99)
    {
        ...
        case 2:
        {
            ...
            if (choice2 == 1)
            {
                for (i = 0; i < 6; i++)
                {
                    product[i].quantity = product[i].quantity + cart[i].quantity;
                    cart[i].quantity = cart[i].quantity - cart[i].quantity;
                }
                for (i = 0; i < 6; i++)
                {
                    printf("Product %d: %d\n", i+1, product[i].quantity);
                    printf("cart : %d\n", cart[i].quantity);
                }
                fflush(stdin);
                getchar();
    ...
    for (int i = 0; i < productQuantity; i++)
    {
        ...
        // if(product[i].quantity != (product[i].quantity - cart[i].quantity))
            printf("\n");
        // printf("\n");
